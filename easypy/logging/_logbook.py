...
class SilentExceptionLoggerMixin():

    def silent_exception(self, message, *args, **kwargs):
        "like ``exception()``, only emits the traceback in debug level"
        kwargs.update(exc_info="DEBUG")
        self.error(message, *args, **kwargs)

    def _log(self, level, args, kwargs):
        traceback_log_level = None

        exc_info = kwargs.pop("exc_info", None)
        if not isinstance(exc_info, (bool, tuple, type(None))):
            # exc_info is the log-level
            traceback_log_level = logbook.lookup_level(exc_info)
            exc_info = sys.exc_info()
        else:
            kwargs.update(exc_info=exc_info)

        # the frame correction is dynamic since we have a variable number of logbook frames
        # above us (log.warning() calls log.warn()...)
        frm = sys._getframe(1)
        frame_correction = kwargs.get("frame_correction", 0)
        for frame_correction in count(frame_correction + 1):
            if not frm:
                break
            elif not any(frm.f_globals.get('__name__', '').startswith(n) for n in ("logbook", "comet.logging")):
                break
            frm = frm.f_back
        kwargs.update(frame_correction=frame_correction)
        ret = super()._log(level, args, kwargs)

        if traceback_log_level:
            super()._log(
                traceback_log_level,
                ("Traceback:",),
                dict(exc_info=exc_info, frame_correction=frame_correction))
